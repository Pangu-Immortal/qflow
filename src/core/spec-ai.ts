 ...
 *
 * 依赖注入：
 * - callAI: CallAIFn  从外部注入 AI 调用函数（SM-6: 消除 await import('./ai-provider.js') 动态 import）
 */
...
import { GEN_FROM_CODE_MAX_FILES, GEN_FROM_CODE_MAX_CHARS, AI_GEN_SPEC_MAX_TOKENS, MERMAID_LABEL_MAX_LENGTH } from '../shared/constants.js'; // v16.0 Q-3: 硬编码值常量化
...
export class SpecAI {
  ...
  async generateFromCodebase(name: string, type: Spec['type'], filePaths: string[]): Promise<Spec> {
    ...
    // 尝试 AI 生成（SM-6: 使用注入的 callAI 而非动态 import）
    let description: string;
    try {
      const response = await this.callAI( // 调用注入的 AI 函数
        `请基于以下代码文件生成技术规格文档（Spec）：\n\n名称: ${name}\n类型: ${type}\n\n${fileContents.join('\n\n')}`,
        { systemPrompt: '你是一位软件架构师。请生成结构化的技术规格文档，使用 Markdown 格式，适当使用 MUST/SHOULD/MAY 等 RFC 2119 关键词。', maxTokens: AI_GEN_SPEC_MAX_TOKENS },
      );
      description = response.content; // AI 生成的内容
    } catch (e) {
      log.warn(`AI 生成 Spec 失败，使用模板: ${(e as Error).message}`);
  ...
  async generateStructuredSpec(name: string, description: string): Promise<{
    ...
    const descriptionForPrompt = description; // 保存描述备用
    const nameForFallback = name; // 保存名称备用
    const prompt = [
      '你是一个需求分析专家。请根据以下自然语言描述生成一个结构化的软件规格文档。',
      '',
      `描述: ${descriptionForPrompt}`,
      '',
      '请生成以下内容（用 Markdown 格式）：',
      '',
      '## 概述',
      '简要描述功能',
      '',
      ...
      '',
      '## [NEEDS CLARIFICATION]',
      '- 列出所有不确定的点，每个用 [NEEDS CLARIFICATION] 标记',
      '',
      '请确保至少包含 1 个用户故事和 1 个验收条件。不确定的部分必须标记 [NEEDS CLARIFICATION]。',
    ].join('\n'); // 拼接 prompt

    let specContent: string; // Spec 内容
    try {
      const result = await this.callAI(prompt, { maxTokens: 2048 }); // AI 生成
      specContent = result.content; // 取内容
    } catch {
      // AI 失败时使用模板
      specContent = [
        '## 概述',
        ...
        '',
        '## 用户故事',
        `- 作为用户，我希望 ${nameForFallback}，以便提高效率`,
        '',
        '## 验收条件',
        `- GIVEN 系统正常运行 WHEN 使用 ${nameForFallback} THEN 功能正常工作`,
        '',
        '## 技术约束',
      ...
      log.warn('generateStructuredSpec: AI 失败，使用模板'); // 降级日志
    }
  ...
  async fastForward(specId: string): Promise<{
    ...
    for (const section of sections) {
      ...
      try {
        const prompt = [
          `请为以下 Spec section 生成规划产物：`,
          ``,
          `Spec: ${spec.name}`,
          `Section: ${section}`,
          `内容摘要: ${spec.content.slice(0, 500)}`,
          ``,
          `请生成以下产物（Markdown 格式）：`,
          `1. **任务分解**: 列出实现该 section 所需的具体任务（checklist 格式）`,
          `2. **测试大纲**: 列出验收测试用例（GIVEN/WHEN/THEN 格式）`,
          `3. **风险点**: 列出潜在风险和缓解措施`,
        ].join('\n'); // 拼接 AI prompt

        const result = await this.callAI(prompt, { maxTokens: 1024 }); // 调用 AI
        ...
        artifacts.push({
          ...
          content: result.content, // AI 生成内容
        });
        log.debug(`fastForward: AI 为 section "${section}" 生成了规划产物`); // 调试日志
      } catch {
        // AI 不可用时使用结构化模板
        log.debug(`fastForward: AI 不可用，为 section "${section}" 使用模板`); // 调试日志
